...
def show_predict_page():
    ...
    if ok:
        X = [city, family, holiday_type, store_nbr, onpromotion, transactions, oil_price, cluster]
        X[:, 0] = le_cat.transform(X[:,0])
        st.write(X[:, 0])
        X[:, 1] = le_cat.transform(X[:,1])
        st.write(X[:, 1])
        X[:, 2] = le_cat.transform(X[:,2])
        st.write(X[:, 2])
        X[:, 3] = num_scalar.transform(X[:,3])
        st.write(X[:, 3])
        X[:, 4] = num_scalar.transform(X[:,4])
        st.write(X[:, 4])
        X[:, 5] = num_scalar.transform(X[:,5])
        st.write(X[:, 5])
        X[:, 6] = num_scalar.transform(X[:,6])
        st.write(X[:, 6])
        X[:, 7] = num_scalar.transform(X[:,7])
        st.write(X[:, 7])
        X = X.astype(float)
        st.write(X)
